...
int display_char(unsigned char c, int x, int y)
{
    int r, bit;
    ...
    for (r = 0; r < 16 * SCALAR; r++)
    {                           // each ASCII char is a 16row x 8col bitmap
        byte = * (caddress + ( r / SCALAR)); // get each byte of a row

        for (bit = 0; bit < 8 * SCALAR; bit++)
        {                               // for each bit in this byte
            if (byte & (1 << ( bit / SCALAR) ))      // set pixel by 1 bits in this byte
                setpix(x + bit, y + r); // x inc, but same y+r
        }
    }
}

int remove_char(unsigned char c, int x, int y)
{
    int r, bit;
    unsigned char *caddress, byte;
    ...
    for (r = 0; r < 16 * SCALAR; r++)
    {
        byte = *(caddress + row/ SCALAR);

        for (bit = 0; bit < 8 * SCALAR; bit++)
        {
            if (byte & (1 << bit/SCALAR)) // OR simply clr all pixels in this row
                clrpix(x + bit, y + r);
        }
    }
}
...
{
    int x, y;
    x = co * 8 * SCALAR; // pixel location (x,y) in 480x640 matrix
    y = ro * 16 * SCALAR;
    display_char(c, x, y);
}
