...
#include "uio.c"
#include "ucode.c"
int main(int argc, char *argv[]) { // no params???
    printf("SUCCESSFULLY ENTERED INIT\n");
    int sh, pid, status;
    sh = fork();

    if (sh) { // wait for ANY child to die
        // P1 runs in a while(1) loop
        printf("inside parent fork processs\n");
        while (1) {
...
//     printf("INIT %d: fork a login proc on console\n", pid);
